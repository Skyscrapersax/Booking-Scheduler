...
Operators can edit or reschedule a booking without cancelling it, inspect prior details, and search all active/cancelled records. Failed sign-ins are throttled in the database across instances and restarts.
...
3. Set `DATABASE_URL` to the provider's pooled connection URI with `sslmode=require`. A project-specific `BOOKING_DATABASE_URL` takes precedence when present.
4. Generate a stable `BOOKING_SECRET` (32+ characters) and `BOOKING_PASSWORD` (16+ characters) in a password manager. A local alternative is `python -c 'import secrets; print(secrets.token_urlsafe(48))'`. Set both as sensitive Vercel environment variables; keep them distinct for each app/environment.
5. Add your exact custom domain as `BOOKING_HOST` (hostname only). Vercel deployment, branch and production hostnames are included from their system variables. No wildcard hosts. Keep `BOOKING_COOKIE_SECURE` unset. `BOOKING_TIMEZONE` defaults to America/New_York.
...
This is an operator desk with one shared password, not public self-booking or a multi-role account system.
